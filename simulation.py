...
class Simulation:
    def __init__(self, width=1500, height=750, visualization=pygame.SHOWN, carVelocity=20, trafficTimes="realTime"):
        ...
        if visualization == pygame.HIDDEN:
            self.simulationProperties = {"Simulation Time": self.simulationTime, "Simulation Sort": "without visualization"}
    ...
    def run(self, currentTime=0):
        ...
        updateTime = UpdateTime(self, self.simulationTime + 30, gen.allCars, trafficSignals, statisticsHandler, self.simulationProperties)
            ...
            for road in gen.allCars:
                ...
                for index in range(len(gen.allCars[road])):
                    ...
                    if car.arrivalTime <= 1:
                        ...
                        self.screen.blit(rotated, pygame.Vector2(car.currentPosition) * self.ppu - (rect.width / 2, rect.height / 2))
...
def getSimulationProperties():
    currentTime = int(input("wich Time [0,19]?      "))
    if not (0 >= currentTime <= 19):
        currentTime = 0
    trafficTimes = int(input("wich trafficProgram: 0 realTime, 1 halfTime, 2 quarterTime?       "))
    if not (0 >= trafficTimes <= 2):
        trafficTimes = 0
    if trafficTimes == 0:
        trafficTimes = "realTime"
    elif trafficTimes == 1:
        trafficTimes = "halfTime"
    else:
        trafficTimes = "quarterTime"


    return currentTime, trafficTimes
# Run the code
if __name__ == '__main__':
    currentTime,  trafficTimes = getSimulationProperties()
    simulation = Simulation(width=1500, height=750, trafficTimes=trafficTimes)
    simulation.run(currentTime=currentTime)
